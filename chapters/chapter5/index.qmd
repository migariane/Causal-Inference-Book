...
**Box 5.1 (R):** IPTW with regression adjustment (IPTW-RA)
```r
# Simulate data
set.seed(123)
n <- 1000
...
**Box 5.2 (R):** AIPTW with linear models
```r
...
**Box 5.3 (R):** Loading and preparing RHC data for TMLE
```r
library(readr)
library(knitr)
...
**Box 5.4 (R):** Step 1: Initial prediction of the outcome (G-computation)
```r
# Step 1
...
**Box 5.5 (R):** Predicting potential outcomes under treatment and control
```r
...
```
**Box 5.1 (Stata):** TMLE by hand (6 steps)
...
**Box 5.6 (R):** Step 2: Estimation of the propensity score
```r
...
g0W = (-1 / (1-gW))
```
**Box 5.2 (Stata):** TMLE with eltmle command
...
**Box 5.7 (R):** Step 3: Computing the clever covariate H(A,W)
```r
...
**Box 5.8 (R):** Step 3b: Estimating epsilon via logistic regression with offset
```r
epsilon <- coef(glm(data2$Y ~ -1 + HAW + offset(QAW), family = "binomial"))
```
**Box 5.3 (Stata):** Data setup for TMLE analysis with RHC dataset
...
**Box 5.9 (R):** Step 4: Updating initial outcome predictions (targeting step)
```r
...
**Box 5.10 (R):** Step 5: Computing the targeted ATE estimate
```r
# Step 5 targeted estimate of the ATE
...
**Box 5.11 (R):** Step 6: Computing the efficient influence curve for standard errors
```r
...
**Box 5.12 (R):** Step 6b: Computing 95% Wald confidence intervals
```r
...
The `eltmle` Stata package (available at [github.com/migariane/eltmle](https://github.com/migariane/eltmle)) provides a complete implementation of TMLE with Super Learner integration for Stata users:

**Box 5.4 (Stata):** Installing eltmle via SSC
...
**Box 5.5 (Stata):** Installing eltmle from GitHub
...
**Box 5.6 (Stata):** Cross-validated TMLE with cveltmle
...
**Box 5.13 (R):** TMLE using the tmle R package with Super Learner
```r
set.seed(1)
...
**Box 5.7 (Stata):** Standard and cross-validated TMLE with eltmle
...
**Box 5.14 (R):** CV-TMLE using the tmle3 R package
```r
library(tmle3)
library(sl3)
