...
head(data)
```
**Box 6.1 (Stata):** IPTW computation for ATE
...
summary(model_naive)
```
**Box 6.2 (Stata):** Bootstrap for IPTW
...
```
**Box 6.4 (Stata):** Data setup for longitudinal analysis
...
summary(data$sw)
```
**Box 6.5 (Stata):** Data preparation for longitudinal analysis

```stata
* Download and prepare RHC dataset
use "rhc.dta", clear

* Define key variables
global Y death_d30                       // Outcome
global A rhc                             // Treatment
...
ATE
```
**Box 6.7 (Stata):** Stata equivalent for Box 6.7
...
summary(result)
```
**Box 6.8 (Stata):** Stata equivalent for Box 6.8

```stata
* LTMLE implementation using eltmle
* ssc install eltmle
* github install migariane/eltmle
eltmle $Y $A $W, tmle
eltmle $Y $A $W, tmle bal   // check balance
```
...
```
**Box 6.10 (Stata):** Stata equivalent for Box 6.10
...
gen double p_uncens = 1 - p_cens
gen double ipcw = 1 / p_uncens
summarize ipcw
```
...
summary(initial_fit)
```
**Box 6.13 (Stata):** Stata equivalent for Box 6.13

```stata
* Survival TMLE overview
* See the companion tutorial at:
* https://github.com/migariane/TutorialComputationalCausalInferenceEstimators
* Key steps implemented via eltmle with survival outcomes:
* eltmle Y A W, tmle
```
